/*!
 * @nuxt/webpack v2.3.4 (c) 2016-2018
...
class PostcssConfig {
  ...
  loadPlugins(config) {
    ...
    if (common.isPureObject(plugins)) {
...
class StyleLoader {
  ...
  normalize(loaders) {
    loaders = common.wrapArray(loaders);
  ...
  styleResource(ext) {
    ...
    if (extResource) {
      const patterns = common.wrapArray(extResource).map(p => path.resolve(this.rootDir, p));
...
class WebpackBaseConfig {
  ...
  output() {
    return {
      ...
      publicPath: common.isUrl(this.options.build.publicPath)
        ? this.options.build.publicPath
        : common.urlJoin(this.options.router.base, this.options.build.publicPath)
    }
  }
...
class WebpackBundler {
  ...
  async build() {
    ...
    Object.keys(styleResources).forEach(async (ext) => {
      await Promise.all(common.wrapArray(styleResources[ext]).map(async (p) => {
    ...
    // Start Builds
    const runner = options.dev ? common.parallel : common.sequence;
