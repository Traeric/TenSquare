/*!
 * @nuxt/config v2.3.4 (c) 2016-2018
...
function getNuxtConfig(_options) {
  ...
  options.globalName = (common.isNonEmptyString(options.globalName) && /^[a-zA-Z]+$/.test(options.globalName))
    ? options.globalName.toLowerCase()
    : `nuxt`;

  // Resolve rootDir
  options.rootDir = common.isNonEmptyString(options.rootDir) ? path.resolve(options.rootDir) : process.cwd();
  ...
  const hasSrcDir = common.isNonEmptyString(options.srcDir);
  const hasGenerateDir = common.isNonEmptyString(options.generate.dir);
  ...
  common.guardDir(options, 'rootDir', 'buildDir');
  ...
  if (hasGenerateDir) {
    ...
    // Protect rootDir against buildDir
    common.guardDir(options, 'rootDir', 'generate.dir');
  }

  if (hasSrcDir) {
    // Protect srcDir against buildDir
    common.guardDir(options, 'srcDir', 'buildDir');

    if (hasGenerateDir) {
      // Protect srcDir against generate.dir
      common.guardDir(options, 'srcDir', 'generate.dir');
    }
  }
  ...
  options.modulesDir = []
    ...
    .concat(path.join(options.nuxtDir, 'node_modules')).filter(common.isNonEmptyString)
  ...
  /* istanbul ignore if */
  if (options.dev && common.isUrl(options.build.publicPath)) {
  ...
  if (options.loadingIndicator) {
    ...
    if (!common.isPureObject(options.loadingIndicator)) {
