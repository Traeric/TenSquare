...
async function _import(modulePath) {
  let m;
  try {
    m = await Promise.resolve(require(path.resolve(localNodeModules, modulePath)));
  } catch (e) {
    m = await Promise.resolve(require(modulePath));
  }
  return m
}
...
class NuxtCommand {
  ...
  async getNuxtConfig(argv, extraOptions) {
    const config$$1 = await loadNuxtConfig(argv);
    const options = Object.assign(config$$1, extraOptions || {});
