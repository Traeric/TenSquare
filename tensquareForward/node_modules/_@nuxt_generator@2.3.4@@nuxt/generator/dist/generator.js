/*!
 * @nuxt/generator v2.3.4 (c) 2016-2018
...
class Generator {
  constructor(nuxt, builder) {
    ...
    this.distNuxtPath = path.join(
      this.distPath,
      common.isUrl(this.options.build.publicPath) ? '' : this.options.build.publicPath
    );
  }
  ...
  async initRoutes(...args) {
    ...
    if (this.options.router.mode !== 'hash') {
      try {
        generateRoutes = await common.promisifyRoute(
    ...
    let routes =
      this.options.router.mode === 'hash'
        ? ['/']
        : common.flatRoutes(this.options.router.routes);
      ...
      await Promise.all(
        routes
          ...
          .map(async ({ route, payload }) => {
            await common.waitFor(n++ * this.options.generate.interval);
  ...
  decorateWithPayloads(routes, generateRoutes) {
    ...
    generateRoutes.forEach((route) => {
      ...
      const path$$1 = common.isString(route) ? route : route.route;
      routeMap[path$$1] = {
        route: path$$1,
